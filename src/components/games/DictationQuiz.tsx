...
export function DictationQuiz({ onExit }: { onExit: () => void }) {
  ...
  useEffect(() => {
    if (level && words.length === 0) {
      const pool = shuffle([...SPELLING[level]]).slice(0, 10);
      setWords(pool);
    }
  }, [level, words.length]);
  ...
  useEffect(() => { if (words.length > 0 && idx < words.length) { setTimeout(speakWord, 300); inputRef.current?.focus(); } }, [idx, words]);
  ...
  function check() {
    ...
    if (val === word) {
      ...
      setFeedback(streak >= 2 ? `Great! \u{1F525} ${streak + 1}!` : 'Correct!');
  ...
  function advance() {
    setInput(''); setFlash(null); setFeedback('');
    if (idx + 1 >= words.length) { setDone(true); } else { setIdx(i => i + 1); }
  }

  if (!level) return <LevelSelect title="Dictation" color="#6a0572" icon="\u{1F50A}" onSelect={(lv) => setLevel(lv as 1|2|3)} onBack={onExit} />;

  if (done) {
    const total = words.length; const pct = correct / total;
    const stars = pct >= 0.9 ? 3 : pct >= 0.6 ? 2 : pct >= 0.3 ? 1 : 0;
    if (pupil) recordGameResult({ pupilId: pupil.id, gameId: 'dictation', score: Math.round(pct*100), stars, streak: 0, bestStreak: streak, correct, total });
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-gradient-to-b from-slate-900 to-purple-900/30">
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 max-w-md w-full text-center border border-white/20">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Dictation Done!</h2>
          <p className="text-purple-300 text-lg">{correct}/{total} correct</p>
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={() => { setLevel(null); setWords([]); setIdx(0); setCorrect(0); setStreak(0); setMissed([]); setDone(false); }} className="flex-1 py-3 bg-purple-500 hover:bg-purple-400 text-white font-bold rounded-xl">Again</button>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-slate-900 to-purple-900/20">
      ...
      <div className="px-4 pt-2"><div className="h-2 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-purple-500 rounded-full transition-all" style={{width:`${(idx/words.length)*100}%`}} /></div></div>
      <div className="flex-1 flex flex-col items-center justify-center px-4 max-w-md mx-auto w-full gap-4">
        <p className="text-white/60 text-sm">Listen and type the word</p>
        <button onClick={speakWord} className="text-5xl animate-pulse">🔊</button>
        ...
        <button onClick={check} className="px-8 py-3 bg-purple-500 hover:bg-purple-400 text-white font-bold rounded-xl active:scale-95">Check</button>
